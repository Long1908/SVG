...
private:
	...
	void enterAttributes(double xAxis, double yAxis, double radius, const char* colour)
	{
		this->xAxis = xAxis;
		this->yAxis = yAxis;
		this->radius = radius;
...
public:
	...
	SVGcircle(double xAxis, double yAxis, double radius, const char* colour)
	{
		enterAttributes(xAxis, yAxis, radius, colour);
	}
	...
	void enterFigureCircle()
	{
		...
		for (int i = 0; i < k; ++i)
		{
			...
			{
				copyFile << "  <circle cx=\"" << this->xAxis << "\" cy=\"" << this->yAxis << "\" radius=\"" << this->radius << "\" fill=\"" << this->colour << "\" />" << endl;
			}
		}
		copyFile << "</svg>";
		...
		index1 << k + 1;
		cout << "Successfully created a Circle!" << endl;
	}

	void withinTheFigure(double x, double y, double radius)
	{
